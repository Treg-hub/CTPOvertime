...
- Updated employee selection in overtime form: Replaced Autocomplete with dropdown-style field opening dialog with search TextField and CheckboxListTile for multi-select, filtered by department selected in overtime list screen. Made clock number search case-insensitive. Added loading indicator and no employees message, fixed firstWhere crash with orElse to prevent app freeze. Now shows all employees (not just unselected), checkboxes selectable, excludes managers (department != 'Manager'). Fixed checkbox toggling with ValueNotifier for reactive updates. Fixed search filtering with ValueNotifier for real-time updates while typing.
